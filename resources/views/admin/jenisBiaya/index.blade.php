...
@section('page-title', 'List Jenis-Biaya')
...
<div class="card">
  <div class="card-header mb-2 justify-content-between">
    ...
    <a href="{{ route('jenis-biaya.create') }}" class="btn btn-success">Tambah data</a>
  </div>
      ...
      <tbody>
        @foreach ($biaya as $index => $item)
          <tr>
            ...
            <td class="text-end">
              <a href="{{ route('jenis-biaya.edit', $item->id) }}" class="btn btn-warning btn-sm">Edit</a>
              <form class="d-inline" action="{{ route('jenis-biaya.destroy', $item->id) }}" method="POST">
                @csrf
                @method('DELETE')
