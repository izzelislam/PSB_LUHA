...
@section('page-title', 'List Persyaratan')
...
<div class="card">
  <div class="card-header mb-2 justify-content-between">
    ...
    <a href="{{ route('persyaratan.create') }}" class="btn btn-success">Tambah data</a>
  </div>
      ...
      <tbody>
        @foreach ($persyaratan as $index => $item)
          <tr>
            ...
            <td>
              <a href="{{ route('persyaratan.edit', $item->id) }}" class="btn btn-warning btn-sm">Edit</a>
              <form class="d-inline" action="{{ route('persyaratan.destroy', $item->id) }}" method="POST">
                @csrf
                @method('DELETE')
